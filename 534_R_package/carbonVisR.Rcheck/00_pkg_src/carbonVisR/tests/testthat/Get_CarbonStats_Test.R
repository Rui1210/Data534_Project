...
test_that("Get_CarbonStats throws an error for invalid dates", {
  # Provide invalid dates (e.g., from_date > to_date)
  from_date_invalid <- "2023-01-30"
  to_date_invalid <- "2023-01-20"

  # Run the function and expect an error
  expect_error({
    result <- Get_CarbonStats(from_date_invalid, to_date_invalid)
  }, class = "error", regexp = "expected error message or pattern")
})
